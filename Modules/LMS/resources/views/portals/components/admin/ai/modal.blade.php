    ...
    <div class="">
        ...
        <div id="ai-content-modal-dragger" class="flex items-center justify-between p-4 border-b dark:border-dark-border cursor-move">
            <div class="card-title text-lg">
                {{ translate('Générateur de contenu IA') }}
            </div>
            <div class="flex items-center gap-2">
                <div aria-label="Indicateur de déplacement de la modale" class="btn btn-sm size-8 text-heading dark:text-dark-text bg-gray-200 dark:bg-dark-icon rounded-lg cursor-move">
                    <i class="ri-drag-move-2-fill text-inherit"></i>
                </div>
                <button type="button" aria-label="Bouton de fermeture de la modale" class="btn btn-sm size-8 text-heading dark:text-dark-text bg-gray-200 dark:bg-dark-icon rounded-lg ai-content-modal-close-btn">
            ...
            <form action="{{ route('generate.content') }}" method="post" class="form mt-2">
                ...
                <label class="form-label block">
                    <select name="service_type_id" class="singleSelect">
                        <option disabled selected>{{ translate('Sélectionnez un type') }}</option>
                ...
                <label class="form-label block">
                    <input type="text" name="keyword" placeholder="{{ translate('Saisissez un mot-clé') }}" class="form-input" />
                    <span class="text-danger error-text keyword_err"></span>
                </label>
                <div class="grid grid-cols-2 gap-2">
                    <label class="col-span-full sm:col-span-1 form-label block">
                        <input type="number" name="max_token" placeholder="{{ translate('Longueur maximale') }}" class="form-input" />
                    </label>
                    <label class="col-span-full sm:col-span-1 form-label block">
                ...
                <button type="submit" class="btn b-solid btn-primary-solid w-full dk-theme-card-square my-3">{{ translate('Générer') }}</button>
                <div class="form-label block">
                    <textarea id="outputContent" class="form-input font-normal edit-ai-content" placeholder="{{ translate('Le contenu généré apparaîtra ici') }}" rows="10" readonly></textarea>
                    <div class="flex items-center justify-end gap-2 mt-1">
                        ...
                        <button type="button" class="btn btn-sm [&.active]:bg-primary [&.active]:text-white copytext" data-copy-button>
                            ...
                            <span class="text">{{ translate('Copier') }}</span>
                        </button>
                    </div>
                </div>
            </form>
