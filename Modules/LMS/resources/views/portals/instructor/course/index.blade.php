@php
    ...
    $textTitle = translate('Voulez-vous déplacer vers la corbeille ?');
    $text = translate('Si vous déplacez vers la corbeille, le cours et toutes les données associées seront mis à la corbeille.');
    $isInstructor = isInstructor();
@endphp


<x-dashboard-layout>
    <x-slot:title> {{ translate('Gestion des cours') }} </x-slot:title>
    <!-- FIL D'ARIANE -->
    <x-portal::admin.breadcrumb title="Mes cours" page-to="Cours"
    ...
    <div class="card">
        ...
        <div class="flex items-center gap-2 pb-5 mb-5 border-b border-gray-200 dark:border-dark-border">
            <a href="{{ route('instructor.course.index', ['filter' => 'all']) }}"
                class="badge badge-primary-outline b-outline group/b-counter is-hover-active rounded-full dk-theme-card-square {{ get_active_filter_tab() === 'all' ? 'active' : '' }}">{{ translate('Tous') }}
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-dark-border-three">
                        @foreach ($courses as $course)
                            @php
                                ...
                                if ($course->trashed()) {
                                    $textTitle = translate('Êtes-vous sûr de vouloir supprimer définitivement ce cours ?');
                                    $text = translate(
                                        'Si vous le supprimez, le cours et toutes les données associées seront supprimées définitivement.',
                                    );
                                }
                                    ...
                                    <div class="flex items-center gap-1">
                                        ...
                                        @if ($course->trashed())
                                            <button data-action="{{ route($restoreRoute, $course->id) }}"
                                                ...
                                                data-title="{{ translate('Voulez-vous restaurer ce cours ?') }}">
