...
<x-auth-layout class="home-online-education" :data="$settings">
    <div class="min-w-full min-h-screen flex items-center">
        ...
        <div class="grow min-h-screen h-full w-full lg:w-1/2 pt-32 pb-12 px-3 lg:p-3 flex-center flex-col">
            <h2 class="area-title">{{ translate('Connexion Organisation') }}!</h2>
            <p class="area-description max-w-screen-sm mx-auto text-center mt-5">
                {{ translate('Connectez-vous en tant qu\'organisation pour accéder à votre espace dédié.') }}
            </p>
            <div class="w-full max-w-screen-sm mt-10">
                <x-theme::form.login-form userType="organization" />
            </div>
            <div
            ...
            <div class="text-heading">
                {{ translate('Vous n\'avez pas encore de compte organisation') }}?
                <a href="{{ route('register.organization') }}" class="text-primary hover:underline"
                    aria-label="Page d'inscription organisation">{{ translate('Inscrivez-vous') }}</a>
            </div>
        </div>
    </div>
</x-auth-layout> 